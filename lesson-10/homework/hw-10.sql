...
-- 6
select e.name EmployeeName, m.name ManagerName from employees e
...
-- 10
select d.departmentname, count(EmployeeID) EmployeeCount
from employees e
join departments d
on e.DepartmentID = d.DepartmentID
...
-- 12
select c.firstname, c.lastname, o.quantity totalorders
from orders o
left join customers c
on o.CustomerID = c.CustomerID
where quantity >= 1
...
-- 14
select e.name employee1, m.name employee2, m.managerid
from employees e
join employees m
...
-- 19
select name employeename, salary from employees e
...
-- 22
select e.name emp1, e.Salary, e2.name emp2, e2.Salary, e.managerid
from employees e
join employees e2
on e.ManagerID = e2.ManagerID
