...
-- 1
select ProductName, price as MinPrice
from products
...
-- 4
select count(distinct Category) as totalCount from Products

-- 5
select ProductID, sum(SaleAmount) as SaleAmount from Sales
where productId = 7
group by ProductID
...
-- 9
select CustomerID, sum(SaleAmount) TotalSale from sales
group by customerid

-- 10
select count(employeeid) as TotalEmp, departmentname from employees
group by DepartmentName
having count(employeeid) > 5

-- Medium 11
select ProductID, avg(saleamount) as Avg, sum(saleamount) as Total from sales
group by ProductID

-- 12
select DepartmentName, count(EmployeeID) NumOfEmp from employees
where  DepartmentName = 'HR'
group by departmentname

-- 13
select DepartmentName Dep, max(salary) HighestSal, min(salary) LowestSal from Employees
group by departmentname

-- 14
select DepartmentName Dep, avg(Salary) AvgSal from Employees
group by departmentname

-- 15
select DepartmentName Dep, count(EmployeeId) NumOfEmp, avg(Salary) AvgSal from employees
group by departmentname
...
-- 17
select year(SaleDate) Year, sum(SaleAmount) TotalSale
from sales
...
-- 19
select DepartmentName Dep, avg(Salary) as 'Avg>60000' from Employees
group by DepartmentName
...
-- 21
select CustomerID, sum(SaleAmount)'Total>1500' from sales
group by CustomerID
having sum(SaleAmount)>1500 

-- 22
select DepartmentName, Avg(Salary) AvgSal, sum(Salary) Total from employees
group by DepartmentName
having avg(salary) > 65000

-- 23
select CustomerID, sum(TotalAmount) Total, min(TotalAmount) LeastPurchase from Orders
group by CustomerID
having sum(TotalAmount) > 50

-- 24
select year(OrderDate) Year, month(OrderDate) Month, count(distinct ProductID) UniqueProducts, sum(TotalAmount) TotalSales
from orders
...
-- 25
select * from orders
select year(OrderDate) Year, min(quantity) MinQuantity, max(quantity) MaxQuantity from orders
