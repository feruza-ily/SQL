...
-- 3
select d.departmentname, max(e.salary) MaxSal
from employees e
join departments d
on e.DepartmentID = d.DepartmentID
...
where s.SupplierName = 'gadget supplies'
...
-- 10
select p.productname, sum(saleamount) totalsalesamount
from products p
join sales s
...
-- 16
select c.categoryname, count(p.category) productcount
from products p
join categories c
...
or name like '[aeiou]%[aeiou]%[aeiou]%[aeiou]'
