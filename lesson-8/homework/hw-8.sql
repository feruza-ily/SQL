...
-- Easy 1
select Category, count(distinct ProductName) NumOfProd from products
group by Category
...
-- 7
select ProductName, StockQuantity, iif (stockquantity  < 30, 'Low Stock', 'Sufficient') StockStatus
from products

-- 8
select Country, count(CustomerID) TotalCus from customers
group by country

-- 9
select * from orders
select min(quantity) MinQuantity,max(quantity) MaxQuantity from orders
...
-- 13
select year(OrderDate) Year, avg(Quantity) AvgQuantity from Orders
...
-- 16
select ProductID, sum(SaleAmount) TotalSales from sales
group by ProductID
...
-- 21
select  ProductName, count(productname) TimesSold from products p
join sales s
